...
void beta_tick(Beta* beta, lua_State *L) {
	...
	char c;
	switch(opcode) {
		case ADD:	beta_write_reg(beta, val_a + val_b, reg_c);		beta->pc += 4; break;
		case ADDC:	beta_write_reg(beta, val_a + literal, reg_c);	beta->pc += 4; break;
		case AND:	beta_write_reg(beta, val_a & val_b, reg_c);		beta->pc += 4; break;
		case ANDC:	beta_write_reg(beta, val_a & literal, reg_c);	beta->pc += 4; break;
		case BEQ:
			beta->pc += 4;
		...
		case CMPLTC:
			...
			beta_write_reg(beta, ((int32_t)val_a < (int32_t)literal)? 1: 0, reg_c);
			break;
		case DIV:	beta_write_reg(beta, val_a / val_b, reg_c);		beta->pc += 4; break;
		case DIVC:	beta_write_reg(beta, val_a / literal, reg_c);	beta->pc += 4; break;
		case JMP:
			beta->pc += 4;
			beta_write_reg(beta, beta->pc, reg_c);
			ea = (val_a & ~0x3);
			if(!(beta->pc & (1<<31)) && (ea & (1<<31))) ea &= 0x7FFFFFFF;
			beta->pc = ea;
			break;
		case LD:
			beta->pc += 4;
		...
		case LDR:
			...
			beta_write_reg(beta, beta_read_mem(beta, ea), reg_c);
			break;
		case MUL:	beta_write_reg(beta, val_a * val_b, reg_c);		beta->pc += 4; break;
		case MULC:	beta_write_reg(beta, val_a * literal, reg_c);	beta->pc += 4; break;
		case OR:	beta_write_reg(beta, val_a | val_b, reg_c);		beta->pc += 4; break;
		case ORC:	beta_write_reg(beta, val_a | literal, reg_c);	beta->pc += 4; break;
		case SHL:	beta_write_reg(beta, val_a << val_b, reg_c);	beta->pc += 4; break;
		case SHLC:	beta_write_reg(beta, val_a << literal, reg_c);	beta->pc += 4; break;
		case SHR:	beta_write_reg(beta, val_a >> val_b, reg_c);	beta->pc += 4; break;
		case SHRC:	beta_write_reg(beta, val_a >> literal, reg_c);	beta->pc += 4; break;
		case SRA:
			res = val_a >> val_b;
		...
		case ST:
			...
			beta->pc += 4;
			break;
		case XOR:	beta_write_reg(beta, val_a ^ val_b, reg_c);			beta->pc += 4; break;
		case XORC:	beta_write_reg(beta, val_a ^ literal, reg_c);		beta->pc += 4; break;
		case XNOR:	beta_write_reg(beta, ~(val_a ^ val_b), reg_c);		beta->pc += 4; break;
		case XNORC: beta_write_reg(beta, ~(val_a ^ literal), reg_c);	beta->pc += 4; break;
